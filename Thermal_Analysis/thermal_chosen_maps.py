...
def plot_annual_occupancy_map(zone_choice: np.ndarray, title: str) -> None:
    ...
    We reserve a fixed right-side column (same as clothing plot colorbar)
    so both plots have identical main-axis width/height.
    """
    ...
    # Reserve a consistent "colorbar column" even though we don't draw one here
    fig, (ax, ax_cb) = plt.subplots(
        nrows=1,
        ncols=2,
        figsize=(11, 4.2),
        gridspec_kw={"width_ratios": [1, 0.045], "wspace": 0.05},
    )
    ax_cb.axis("off")  # keep space, but hide
...
def plot_clothing_map(clo_Z: np.ndarray, title: str) -> None:
    ...
    Colorbar is placed in a fixed right-side column so the main axes
    match the occupancy map exactly.
    """
